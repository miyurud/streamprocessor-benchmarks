...
def save_line_chart(chart, column, sleep_time, title):
    ...
    sns_plot = sns.pointplot(x=" Total number of events received (non-atomic)", y=column, hue="Message Size (Bytes)", data=df.loc[df['Sleep Time (ms)'] == sleep_time], ci=None, dodge=True)
    plt.suptitle(title)
...
for sleep_time in unique_sleep_times:
    save_line_chart("thrpt", " Throughput in this window (events/second)", sleep_time, "Throughput (Requests/sec) vs Concurrent Users for " + str(sleep_time) + "ms backend delay")
